    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="card max-w-sm w-full p-6 relative" onClick={e => e.stopPropagation()} data-testid="crew-profile-modal">
        ...
        <div className="text-center mb-4">
          <div className="w-20 h-20 rounded-full overflow-hidden bg-slate-800 flex items-center justify-center mx-auto border-4 border-sky-400 mb-3">
            {photo ? (
        ...
        {profile.recent_ratings?.length > 0 && (
          <div className="mb-4">
            ...
            {profile.recent_ratings.slice(0, 2).map((r, i) => (
              <div key={r.rated_by || r.created_at || i} className="bg-slate-50 dark:bg-slate-800 rounded-lg p-2 mb-1">
...
export default function ContractorDashboard() {
  ...
  const sortedJobs = [...jobs].sort((a, b) => {
    if (jobSort === "name") return (a.title || "").localeCompare(b.title || "");
                ...
                <div key={job.id} className="space-y-1">
                  ...
                  <div className="flex items-center gap-1 px-1 pb-1">
                    ...
                    {job.crew_pending?.length > 0 && (
                      <button
                        onClick={() => {
                          setApplicantsJob(applicantsJob === job.id ? null : job.id);
                          if (applicantsJob !== job.id) fetchApplicants(job.id);
                        }}
                    ...
                    {/* Cancel requests badge */}
                    {job.cancel_requests?.length > 0 && (
                      <button
                        onClick={() => setCancelReqJob(cancelReqJob === job.id ? null : job.id)}
                  ...
                  {applicantsJob === job.id && (
                    <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-2.5 space-y-2 border border-amber-200 dark:border-amber-700" data-testid={`applicants-panel-${job.id}`}>
                      ...
                      ) : applicantDetails[job.id].map(crew => (
                        <div key={crew.id} className="flex items-center justify-between gap-2">
                          <div className="min-w-0">
                            <p className="text-xs font-bold text-slate-800 dark:text-white truncate">{crew.name}</p>
                            <p className="text-[10px] text-slate-500">{crew.trade || "General"} · ⭐ {crew.rating?.toFixed(1) || "New"}</p>
                          </div>
                          <div className="flex gap-1 flex-shrink-0">
                            <button onClick={() => approveApplicant(job.id, crew.id)}
                              className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-500 text-white hover:bg-emerald-600 transition-colors"
                              data-testid={`approve-${job.id}-${crew.id}`}>Approve</button>
                            <button onClick={() => declineApplicant(job.id, crew.id)}
                              className="px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-600 hover:bg-red-200 transition-colors"
                              data-testid={`decline-${job.id}-${crew.id}`}>Decline</button>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  {showAddressSuggestions && addressSuggestions.length > 0 && (
                    <ul className="absolute z-[11] left-0 right-0 top-full mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                      {addressSuggestions.map((s, i) => (
                        <li key={s.full_address || i} onMouseDown={() => selectAddressSuggestion(s)}
